(results, waaila) => {
    ...
    */
	...
	if (eventCategories.length > 1) {
		...
		waaila.message('4. It is recommended to add the selected category to the test name.');
		waaila.message('');
        ...
        * @score 0
        */
        waaila.message('This test requires further configuration. The following table contains all available event category-action combinations from last 28 days.');
		waaila.message('<i>The configuration steps are below the table.</i>');
		...
		waaila.message('2. Remove from requiredEventCombinations array all event category-action combinations that do not have to be present every day.');
		waaila.message('3. You can add additional event category-action combinations to the allowedEventCombinations array.');
		waaila.message('');
		eventsLast28days.forEach(row => {
        ...
        */
        requiredEventCombinations.forEach(comb => {
			...
			eventsYesterday.forEach(row => {
				if ((row['eventCategory'] === comb['eventCategory']) && (row['eventAction'] === comb['eventAction'])) {
		...
		const fail_message_blackout = '1) There are some required event category-action combinations missing in the data yesterday: ';
        ...
        */
        eventsYesterday.forEach(row => {
			...
			allowedEventCombinations.forEach(comb => {
				if (((row['eventCategory'] === comb['eventCategory']) && (row['eventAction'] === comb['eventAction'])) | (row['totalEvents'] <= maxCountUnexpected)) {
        ...
        */
        eventsYesterday.forEach(row1d => {
			eventsLast28days.forEach(row28d => {
				if ((row1d['eventCategory'] === row28d['eventCategory']) && (row1d['eventAction'] === row28d['eventAction'])) {
