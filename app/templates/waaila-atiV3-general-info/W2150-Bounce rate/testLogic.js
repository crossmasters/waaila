(results, waaila) => {
    ...
    * @score 70
    */
    const bounceRateSummary = waaila.functions.summarizeAtiResult(results[0]);
    const totalbounceRate = Math.round(+bounceRateSummary['Bounce rate']['total']*10000)/100;
    const assert_pass_message = `Bounce Rate is within expected bounds ${lowerThreshold} % and ${upperThreshold} %`;
    const assert_fail_message = `Bounce Rate is not between ${lowerThreshold} % and ${upperThreshold} % (it is ${totalbounceRate})`;
