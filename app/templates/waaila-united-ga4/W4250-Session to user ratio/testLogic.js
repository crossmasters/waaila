(results, waaila) => {
    ...
    * @score 150
    */
    const sessionUserRatio = waaila.functions.normalizeGaResult(results);
    const sessionUserRatioSummary = waaila.functions.summarizeGaResult(results);
    ...
    const assert_fail_message = `There is an unusually low number of users compared to visits for some devices: ${selectedDeviceArrayJoined} (the share of sessions to users is below the threshold of ${sessionUserThreshold})`;
    ...
    waaila.assert(typeof selectedDeviceArray[0] === 'undefined', 150)
        ...
        .fail.message(assert_fail_message + '<br/><br/> How to fix: <br/>' +  assert_fail_howtofix 
            + '<br/>' + assert_fail_tabledescribe).table(sessionUserRatio);

	//waaila.table(sessionUserRatio);
}