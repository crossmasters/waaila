(results, waaila) => {
    ...
    */
    ...
    /**
    * @assert 1 - Analyze presence of anomalies in sessions aggregate
    * @score 80
    */
    const inputDataHostname = waaila.functions.normalizeGaResult(results);
    const dateList = [...new Set(inputDataHostname.map(row => row['date']))];
    const inputDataAgg = waaila.functions.normalizedData([]);
    const valueColumnName = anomalyDetectionConfigAgg['valueColumn'];
    dateList.forEach(date => {
        const pushObject = {'date': date};
        pushObject[valueColumnName] = inputDataHostname.filter(row => row['date'] === date).map(row => row[valueColumnName]).reduce((partialSum, a) => partialSum + a, 0);
        inputDataAgg.push(pushObject);
    });
