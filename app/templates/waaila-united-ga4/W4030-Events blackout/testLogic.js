(results, waaila) => {
    ...
    */
    ...
    const assert_pass_message = 'Events were measured last 7 days.';
    const assert_fail_message = 'There was no data for some of last 7 days!';
    waaila.assert(sessions.filter(row => row['eventCount'] === 0).length === 0, 150)
        .pass.message(assert_pass_message)
        .fail.message(assert_fail_message)
        .table(sessions, [{ column: 'eventCount', cellColor: { condition: { GREATER_THAN: 0 } } }]);
}