(results, waaila) => {
    ...
    * @score 50
    */
    const referrals = waaila.functions.normalizeGaResult(results);

	const dateYesterday = referrals.map(row => row['date']).reverse()[0];
	const previous = referrals.filter(row => row['date'] !== dateYesterday);
	const current = referrals.filter(row => row['date'] === dateYesterday);
